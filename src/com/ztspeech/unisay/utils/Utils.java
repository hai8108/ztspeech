...
public class Utils {
	...
	public static String buildFinalFilePath(String id, String url) {
		if (url != null && url.startsWith(Configs.ztspeechMusicPath)) {
			return url;
		} else {
			if ("-1".equals(id)) {
				return url;
			} else {
				return Configs.ztspeechMusicPath + id + "." + buildFileType(url);
			}
		}
	}
			...
			try {
				...
				try {
					...
					if (imei == null) {
						...
						LogInfo.LogOut("imei<-not tm.getDeviceId()" + imei);
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
	...
	public static String getIMEI() {
		TelephonyManager tm = (TelephonyManager) com.ztspeech.unisay.trans.main.Application.application
	...
	public static String returnNowTime() {
		...
		try {
			...
			timeString = sdformat.format(date);
	 ...
	 * @author haitian
	 */
	public static String returnSubDaysDate(String currDay, int dayAgo) {
		String timeString = "";
		try {
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
			Date date = sdf.parse(currDay);
			long day = date.getTime() - dayAgo * 60 * 60 * 24 * 1000;
			date = new Date(day);
	...
	public static void updateNotify(Context context, Class<?> c) {
		...
		if (c != null) {
			...
			notification.setLatestEventInfo(context, context.getText(R.string.app_name),
					context.getText(R.string.app_name), contentIntent);
	...
	private final static String[] RANDOM_VEERI_CODE = { "0", "1", "3", "4", "5", "6", "7", "8", "9", "a", "b", "c",
			"d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "q", "r", "s", "t", "u", "v", "w", "x", "y",
			"z" };
	 ...
	 * @return
	 */
	public static boolean saveVoiceData(String fileName, byte[] voice) {
		...
		try {
			...
			fileOutputStream = new FileOutputStream(temp, true);
